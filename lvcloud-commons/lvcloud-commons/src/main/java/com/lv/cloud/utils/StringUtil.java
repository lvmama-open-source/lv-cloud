...
public class StringUtil { 
	...
	public static String filterOutHTMLTags(String inputString) {
		...
		try {
			String regEx_a="<\\s*a.*?/a\\s*>";//定义<a href="">百度</a>样式标签
			String regEx_script = "<[\\s]*?script[^>]*?>[\\s\\S]*?<[\\s]*?\\/[\\s]*?script[\\s]*?>"; // 定义script的正则表达式{或<script[^>]*?>[\\s\\S]*?<\\/script>																						// }
			String regEx_style = "<[\\s]*?style[^>]*?>[\\s\\S]*?<[\\s]*?\\/[\\s]*?style[\\s]*?>"; // 定义style的正则表达式{或<style[^>]*?>[\\s\\S]*?<\\/style>
			String regEx_html = "<[^>]+>"; // 定义HTML标签的正则表达式

			htmlStr=htmlStr.replaceAll(regEx_a, "");//过滤a标签
     ...
     * @return
     * @author ltwangwei
     * @date 2016-6-15 上午10:43:07
     * @since  CodingExample　Ver(编码范例查看) 1.1
     */
    public static boolean intIsNotNull(Integer intValue){
     ...
     * @since  CodingExample　Ver(编码范例查看) 1.1
     */
    public static boolean intIsNull(Integer intValue){
		return !intIsNotNull(intValue);
	}
